...
public class CarrinhoImp implements CarrinhoDAO {
	...
	public void create(Carrinho carrinho) throws Exception {
		...
		stm = conn.prepareStatement("insert into carrinho(cdfuncionario, cdvenda, cdproduto, quantidade) VALUES (?, ?, ?, ?)");
	...
	public Collection<Carrinho> list() throws Exception {
		...
		while(rs.next()) {
			...
			carrinhos.add(new Carrinho(cdFuncionario, cdvenda, cdProduto, quantidade));
		}
		return carrinhos;
	}
